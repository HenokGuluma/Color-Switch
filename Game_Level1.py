...
class Circle:
    def __init__(self, surface, vel, rad, x=250, y=150, angle = 0):
        self.x = x
        self.y = y
        self.rad = rad
        self.angle = angle
...
class Prize:
    ...
    def draw(self):
        ...
        if(not self.over):
            ...
            #When the star is eaten then the +5 mark will appear and fade eventually, the fading is accomplished by using a counter to control the color. 
            self.surface.blit(font.render("+5", True, (255-self.over_count*5, 255-self.over_count*5, 255-self.over_count*5)), (x-10,y-self.over_count))
...
def restart(): 
    ...
    randno=[[-1, 220], [1, 220]]
    #This is just to establish two ways of the circles rotation(clockwise and couterclockwise)
    for i in range(100):
        r=randno[random.randint(0, 1)]
        s=r[0]
        t=r[1]
        temp = Circle(screen, s, t, WID/2, -500*i)
...
def draw_menu():
    screen.fill((0, 0, 0))
    bg=pygame.image.load(path+"picture4.png").convert_alpha()
    #This displays the menu. 
    screen.blit(bg, (0, 0))
def draw_game_over():
    #when the game is over this picture is displayed along with score and highscore. 
    screen.fill((0, 0, 0))
    bg=pygame.image.load(path+"picture3.png").convert_alpha()
    screen.blit(bg, (0, 0))
